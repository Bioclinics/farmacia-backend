...
export class DashboardService {
  ...
  private getDayBounds(date?: string): { start: Date; end: Date; value: string } {
    const base = date && /^\d{4}-\d{2}-\d{2}$/.test(date) ? date : new Date().toISOString().slice(0, 10);
    return {
      value: base,
  ...
  async getKpis(date?: string) {
    ...
    const [salesToday, revenueToday, salesThisMonth, productsStats, ioToday] = await Promise.all([
      ...
      this.dataSource.query(
        `SELECT
            (SELECT COALESCE(SUM(quantity), 0)::int FROM products_inputs WHERE created_at BETWEEN $1 AND $2) AS inputs,
  ...
  async getAlerts(limit = 30) {
    ...
    return this.dataSource.query(
      `SELECT
       ...
       FROM products p
       WHERE p.is_deleted = false AND p.stock <= p.min_stock
  ...
  async getSalesTrend(startDate?: string, endDate?: string) {
    const from = startDate && /^\d{4}-\d{2}-\d{2}$/.test(startDate) ? startDate : new Date().toISOString().slice(0, 10);
    ...
    return this.dataSource.query(
      `SELECT
       ...
       FROM sales s
       WHERE s.created_at BETWEEN $1::timestamp AND $2::timestamp + interval '23 hours 59 minutes 59 seconds'
  ...
  async getTopProducts(startDate?: string, endDate?: string, limit = 5) {
    ...
    return this.dataSource.query(
      `SELECT
       ...
       WHERE po.created_at BETWEEN $1::timestamp AND $2::timestamp + interval '23 hours 59 minutes 59 seconds'
